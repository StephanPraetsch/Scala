...
class ListsSuite extends FunSuite {
  ...
  test("sum of negative and positive numbers") {
    assert(sum(List(1, 2, 0)) === 3)
    assert(sum(List(-1, 4, -2, 0, 1)) === 2)
  }
  test("sum of empty list should be zero") {
    assert(sum(List()) === 0)
  }
  test("sum of null list should throw NPE") {
    intercept[NullPointerException] {
      sum(null)
    }
  }

  test("max of null list should throw NPE") {
    intercept[NullPointerException] {
      max(null)
    }
  }
  test("max of empty list should throw NoSuchElementException") {
  ...
  test("max of a few numbers") {
    ...
    assert(max(List(3, 7, 3, 7, 9, 2, 9)) === 9)
  }
